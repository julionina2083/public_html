        ...
        if ($registered_version != $this->module->version) {
            ...
            if ($overrides) {
                ...
                if (Shop::isFeatureActive() &&
                    (!empty(Shop::getContextShopID()) || !empty(Shop::getContextShopGroupID()))
                ) {
                    $this->module->html = '
                        <div class="bootstrap panel">
                            <div class="alert alert-warning">
                            '.$this->module->l('In order to do the module version update process, it is necessary to do it with "All shops" selected.', self::NAME_CLASS).'
                            </div>
                        </div>
                    ';
                ...
                $this->context->smarty->assign(array(
                    ...
                    'message_all_store' => $this->module->l('You need to update the version of our module for all your stores, please select the option all stores to perform this process', self::NAME_CLASS)
                ));
            }

            $this->module->html = $this->context->smarty->fetch(_PS_MODULE_DIR_.$this->module->name
