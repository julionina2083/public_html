...
class OnePageCheckoutPS extends Module
{
    ...
    public $configure_vars = array(
        ...
        array('name' => 'OPC_PRESEL_CREATE_ACCOUNT', 'default_value' => 0, 'is_html' => false, 'is_bool' => true),
        array(
        ...
        array('name' => 'OPC_MARK_CHECKBOX_CHANGE_PASSWD', 'default_value' => 0, 'is_html' => false, 'is_bool' => true),
        ...
        array(
            ...
            'default_value' => 'envialacarrier,bbcarrier',
            'is_html'       => false
        ),
        array(
            'name'          => 'OPC_MODULE_CARRIER_NEED_CITY',
            'default_value' => 'shippingconfiguratorpro,chilexpress_oficial',
            'is_html'       => false
        ),
        /* payment - step 3 */
    ...
    public function install()
    {
        ...
        if (!parent::install() ||
            ...
            !$this->registerHook('actionObjectCustomerAddressDeleteAfter') ||
    ...
    {
        Db::getInstance(_PS_USE_SQL_SLAVE_)->execute('DROP TRIGGER IF EXISTS `after_delete_module`');
        ...
        if ($this->context->cart->nbProducts() > 0) {
            if (!$this->context->customer->isLogged()) {
                ...
                if (!$this->context->customer->getAddresses($this->context->cookie->id_lang)) {
                    ...
                    if (!$this->cps || ($this->cps && !$this->cps_selected)) {
                        $this->context->cart->id_address_delivery = $id_address;
                        $this->context->cart->id_address_invoice = $id_address;
                        $this->context->cart->update();
                    }
    ...
    public function saveCustomConfigValue($option, &$config_var_value)
    {
        ...
        switch ($option['name']) {
            ...
            case 'enable_terms_conditions':
                if ($config_var_value) {
                    $config_var_value = (int) $this->validValuesConfigVars(Tools::getValue('id_cms_temrs_conditions'), $warn_acceptance);
            ...
            case 'enable_privacy_policy':
                if ($config_var_value) {
                    $config_var_value = (int) $this->validValuesConfigVars(Tools::getValue('id_cms_privacy_policy'), $warn_acceptance);
    ...
    protected function displayForm()
    {
        ...
        $this->params_back = array(
            ...
            'url_contact_presteam'                 => $this->context->language->iso_code === 'es' ? 'http://www.presteamshop.com/es/contactenos' : 'http://www.presteamshop.com/en/contact-us',
    ...
    public function hookActionObjectCartUpdateAfter()
    {
        ...
        return $this->hookActionObjectCustomerAddressUpdateAfter();
    }

    public function hookActionObjectAddressUpdateAfter()
    {
        $this->hookActionObjectCustomerAddressUpdateAfter();
    }
...
//                }
    ...
    public function hookActionObjectCustomerUpdateAfter()
    {
        if ($this->context->controller instanceof FrontController
            && $this->context->controller->controller_type !== 'admin'
        ) {
            return false;
        }
...
//                }
            ...
            if (is_array($groups_availables) && count($groups_availables) > 0) {
                foreach ($groups as $key => $group) {
                    if (!in_array($group['id_group'], $groups_availables)) {
                        if ($this->context->customer->isLogged() && (int)$this->context->customer->id_default_group === (int)$group['id_group']) {
                            continue;
                        }
                        unset($groups[$key]);
                    }
            ...
            if ($field->object == $this->globals->object->customer) {
                ...
                if ($field->name == 'id_gender') {
                    ...
                    if (Configuration::get('PS_GUEST_CHECKOUT_ENABLED')) {
                        ...
                        $new_field->size          = '0';
                        $new_field->type_control  = 'checkbox';
                        $new_field->default_value = (!$this->context->customer->isLogged() && !$this->context->customer->isGuest()) ? (int)$this->config_vars['OPC_PRESEL_CREATE_ACCOUNT'] : '0';
                        $new_field->required      = false;
                        $new_field->is_custom     = false;
                        $new_field->active        = true;
    ...
    {
        $this->hookActionObjectCustomerAddressAddAfter($params);
    }
    ...
    {
        $this->hookActionObjectCustomerAddressDeleteAfter($params);
    }
...
//                && !Tools::getIsset('token')
    ...
    private function updateCustomerAddressDefault($id_customer, $id_address, $object)
    {
        $query = 'UPDATE '._DB_PREFIX_.'opc_customer_address'
            .' SET `default` = 0'
    ...
    public function loadAddressesCustomer()
    {
        ...
        $params = array(
            ...
            'cps' => (bool) ($this->cps !== false && $object === 'delivery'),
            'cps_show_address_message' => (bool) $this->cps_selected
        );
    ...
    private function validateFieldsAddress(&$fields, &$address, $object)
    {
        ...
        if ($address->id_country) {
            // Check country
            if ((!($country = new Country($address->id_country)) || !Validate::isLoadedObject($country)) && $create_address) {
                $this->errors[] = $this->l('Country cannot be loaded.');
            }
    ...
    private function loginCustomerOPC(&$set_id_customer_opc)
    {
    ...
    private function logoutCustomerOPC()
    {
    ...
    public function loadCarrier($order_controller)
    {
        ...
        if (!$this->context->cart->isVirtualCart()) {
            if ($this->context->customer->isLogged()) {
                $is_logged = true;
                $address_customer = $this->context->customer->getAddresses($this->context->cart->id_lang);
                if (empty($address_customer) && !$this->cps_selected) {
                ...
                if (!$is_necessary_city) {
                    if ($this->config_vars['OPC_FORCE_NEED_CITY']) {
                        ...
                        foreach ($carriers_city as $carrier) {
                            if ($this->core->isModuleActive($carrier)) {
                                //No exigimos seleccionar una ciudad si ambos modulos estan activos
                                if ($carrier == 'shippingconfiguratorpro' && $this->cps) {
                                    continue;
                                }
        ...
        $templateVars = array(
            ...
            'title' => '',
            'delivery_options' => $delivery_options,
            'cps_message' => (!$is_virtual_cart && !empty($this->cps_selected) && empty($address_customer) && $is_logged) ? true : false
        );
    ...
    public function loadPayment()
    {
        ...
        //Support module: cgma - v1.6.8 - MyPresta.eu
        $cgma = $this->core->isModuleActive('cgma');
        if ($cgma) {
            $currency = Currency::getCurrency((int)$this->context->cart->id_currency);
            $tinc = $this->l('(tax incl.)');
            $texl = $this->l('(tax excl.)');
            $minimal_purchase = Tools::convertPrice($cgma::returnCustomerGroupMinimum(), $currency);

            if (Configuration::get('cgma_free') == 1) {
                if (($this->context->cart->getOrderTotal((Configuration::get('cgma_tax') == 1 ? true : false), Cart::ONLY_PRODUCTS) - $cgma->returnVirtualValue()) != 0) {
                    if (($this->context->cart->getOrderTotal((Configuration::get('cgma_tax') == 1 ? true : false), Cart::ONLY_PRODUCTS) - $cgma->returnVirtualValue()) < $minimal_purchase) {
                        $msg = sprintf($this->l('A minimum purchase total of %1s %2s is required to validate your order, current purchase total is %2s %3s.'), Tools::displayPrice($minimal_purchase, $currency), (Configuration::get('cgma_tax') == 1 ? ' ' . $tinc : ' ' . $texl), Tools::displayPrice($this->context->cart->getOrderTotal((Configuration::get('cgma_tax') == 1 ? true : false), Cart::ONLY_PRODUCTS) - $cgma->returnVirtualValue(), $currency), (Configuration::get('cgma_tax') == 1 ? ' ' . $tinc : ' ' . $texl));
                        return '<p class="alert alert-warning">'.$msg.'</>';
                    }
                }
            } else {
                if (($this->context->cart->getOrderTotal((Configuration::get('cgma_tax') == 1 ? true : false), Cart::ONLY_PRODUCTS) - $cgma->returnVirtualValue()) < $minimal_purchase) {
...
//        }
    ...
    public function loadReview()
    {
        ...
        if ($module !== false) {
            ...
            if ($free_shipping > 0) {
                $free_ship = Tools::convertPrice((float)$free_shipping, new Currency($this->context->cart->id_currency));
            }
        } else {
        ...
        if ($presented_cart['subtotals']['products']['amount'] !== $total_products_wt) {
            $this->context->smarty->assign('total_products_wt', Tools::displayPrice($total_products_wt, $this->context->currency));
        }
    ...
    public function addModulesExtraFee()
    {
        ...
        if (isset($presented_cart["totals"]["total_including_tax"]["amount"]) && $presented_cart["totals"]["total_including_tax"]["amount"]) {
            $total = $presented_cart["totals"]["total_including_tax"]["amount"];
        } else if (isset($presented_cart["totals"]["total"]["amount"]) && $presented_cart["totals"]["total"]["amount"]) {
        ...
        if ($module !== false) {
            ...
            if (method_exists($module, 'getFee') && ($module->author == 'presta-apps' || $module->author == 'Presta-Apps Solutions')) {
                ...
                if (version_compare($module->version, '3.3.4', '>=')) {
                    $fee = (float)Tools::ps_round((float)$module->getFeeCost($this->context->cart, $codfeeconf, true), 2);
        ...
        //compatibilidad con modulo mrshopcashondelivery - v1.0.9 - Mr. APPs
        $module = $this->core->isModuleActive('mrshopcashondelivery');
        if ($module !== false) {
            $fee = Fee::getFee($this->context->cart, $this->context->language->id);
            if ($fee) {
                $fee_amount = Fee::getFeeAmount($this->context->cart, $fee);
                $payment_modules_fee['mrshopcashondelivery'] = array(
                    'reload' => true,
                    'id' => $module->id,
                    'label_fee' => $label_fee,
                    'label_total' => $label_total,
        ...
        //support module: shaim_cashondelivery - v1.8.8 - Dominik
        $module = $this->core->isModuleActive('shaim_cashondelivery');
        if ($module !== false) {
            $scod_fee = $module->payredCarriers(array('cart' => $this->context->cart));

            $payment_modules_fee['shaim_cashondelivery'] = array(
                'reload' => true,
                'id' => $module->id,
                'label_fee' => $label_fee,
                'label_total' => $label_total,
        ...
        //support module: codwfeeplus - v1.1.8 - Sakis Gkiokas
        $module = $this->core->isModuleActive('codwfeeplus');
        if ($module !== false) {
            $fee = $module->getCost(array('cart' => $this->context->cart));

            $payment_modules_fee['codwfeeplus'] = array(
                'reload' => true,
                'id' => $module->id,
                'label_fee' => $label_fee,
                'label_total' => $label_total,
        ...
        //support module: awredsys - v17.1.1 - www.alabazweb.com
        $module = $this->core->isModuleActive('awredsys', 'getCuota');
        if ($module !== false) {
            $tpvs = AwRedsysTpv::getTpv($this->context->shop->id, 0);
            if ($tpvs) {
                $tpv = new AwRedsysTpv($tpvs[0]['id_tpv']);
                $module->_tpv = $tpv;
                $fee = $module->getCuota($this->context->cart);

                if ($fee > 0 || $fee < 0) {
                    $payment_modules_fee['awredsys'] = array(
                        'id' => $module->id,
                        'label_fee' => $label_fee,
                        'label_total' => $label_total,
                        'fee' => Tools::displayPrice($fee),
        ...
        /*$module = $this->core->isModuleActive('ets_payment_with_fee');
        if ($module !== false) {
            $sql = new DbQuery();
            ...
            if ($payment_modules) {
                foreach ($payment_modules as $payment_module) {
                    $fee = $module->getFeePaymentModule($payment_module['name'], $total);

                    $payment_modules_fee[$payment_module['name']] = array(
                        'reload' => true,
            ...
            if ($ets_payment_cart) {
                if ($ets_payment_cart['ets_payment_module_name'] == 'ets_payment_with_fee') {
                    $fee = $module->getFeePayment($ets_payment_cart['id_payment_method'], $total);

                    $payment_modules_fee['ets_payment_with_fee'] = array(
                        'reload' => true,
                        'id' => $module->id,
                        'label_fee' => $label_fee,
                        'label_total' => $label_total,
...
//                'id_address_delivery' => $address_delivery->id,
    ...
    public function deleteEmptyAddressesOPC()
    {
        ...
        if (count($carts) > 0) {
            foreach ($carts as $cart) {
                ...
                if (Validate::isLoadedObject($customer) && $customer->firstname == 'OPC PTS Not Delete') {
    ...
    {
        $modules = array();
        if ($klarnapaymentsofficial = $this->core->isModuleActive('klarnapaymentsofficial')) {
            $modules['klarnapaymentsofficial'] = $klarnapaymentsofficial->version;
        }
    ...
    public function checkVATNumber()
    {
        ...
        if (!empty($vat_number)) {
            ...
            if ($vatnumbercleaner) {
                $found_module = true;
                if (version_compare($vatnumbercleaner->version, '1.4.6', '<')) {
