        ...
        if (OnePageCheckoutPS.CONFIGS.OPC_AUTOCOMPLETE_GOOGLE_ADDRESS && !$.isEmpty(OnePageCheckoutPS.CONFIGS.OPC_GOOGLE_API_KEY) && typeof google.maps.places !== typeof undefined) {
            if ($('#delivery_address1').length > 0) {
                ...
                if ($('#delivery_id_country').length > 0) {
                    $('#delivery_id_country option').each(function (i, item) {
                        var iso_code = $(item).data('iso-code');
                        if (typeof iso_code !== typeof undefined) {
                            countries.push(iso_code);
                        }
                    });
                } else {
            ...
            if ($('#invoice_address1').length > 0) {
                ...
                if ($('#invoice_id_country').length > 0) {
                    $('#invoice_id_country option').each(function (i, item) {
                        var iso_code = $(item).data('iso-code');
                        if (typeof iso_code !== typeof undefined) {
                            countries.push(iso_code);
                        }
        ...
        $("div#onepagecheckoutps #onepagecheckoutps_step_three")
            .on('click', '.module_payment_container', function (event) {
                if (!$(event.target).hasClass('payment_radio')) {
                    $(event.currentTarget).find('.payment_radio').prop('checked', true).trigger('change');
            ...
            .on("change", "input[name=payment-option]", function () {
                ...
                if (typeof ets_cookie_module_name !== typeof undefined) {
                    ...
                    $.ajax({
                        url: prestashop.urls.pages.order + '?rand=' + new Date().getTime(),
                        data: 'ets_set_payment_option=1&module_name=' + PaymentOPC.name_module_selected + '&id_payment_method=' + id_payment_method + '&payment_option=' + PaymentOPC.id_payment_selected,
                        type: 'post',
                        dataType: 'json',
                        complete: function() {
                            Review.display();
    ...
    getByCountry: function (params) {
        ...
        var _json = {
            ...
            complete: function () {
                ...
                //Compatibilidad: klarnapaymentsofficial - v2.0.5 - Prestaworks AB
                if (typeof initiateKlarnaWidget !== typeof undefined) {
                    var _script = document.createElement('script');
                    _script.type = 'text/javascript';
                    _script.src = prestashop.urls.base_url + 'modules/klarnapaymentsofficial/views/js/front.js';
                    $("body").append(_script);
                }
                //support module ps_checkout - V1.3.0 de PrestaShop
                if (typeof initPsCheckout !== typeof undefined) {
                    var reload_ps_checkout = setInterval(function () {
                        if (window.paypalSdkPsCheckout !== undefined) {
                            initPsCheckout();
                            clearInterval(reload_ps_checkout);
                        }
                    }, 200);
                }
...
var Review = {
    ...
    launch: function () {
        ...
        AppOPC.$opc
            ...
            .on("change", '[name="conditions_to_approve[terms-and-conditions]"]', function (e) {
    ...
    getFields: function (params) {
        ...
        var valid_delivery_address = true;
        if (!is_virtual_cart) {
            if (typeof CarrierPickupStore !== typeof undefined && CarrierPickupStore.id_carrierpickupstore == Carrier.getIdCarrierSelected()
                && !$('form#form_address_delivery').is(':visible')
            ) {
        ...
        $selector.each(function (i, field) {
            ...
            if (object == 'delivery' && !valid_delivery_address) {
                return true;
            }
                    ...
                    success: function (data) {
                        ...
                        if (data.hasError) {
                            ...
                            if (param.validate_payment === true) {
                                ...
                                var arr_reload_payment_modules = ['sofortbanking', 'redsys', 'eupago_multibanco', 'eupago_payshop', 'eupagomultibanco', 'redsysoficial', 'ogone', 'gestpay'];
                                ...
                                var callback_placeorder = function () {
                                    ...
                                    var callback_checkOffer = function () {
                                        if ($payment_selected.data('force-display') === 1) {
                                            document.location.href = prestashop.urls.pages.order + '?mp=' + name_payment;
                                        ...
                                        if (typeof form_payment_selected !== typeof undefined) {
                                            if (name_payment == 'culqi' && typeof Culqi !== typeof undefined) {
                                                ...
                                                //Compatibilidad: klarnapaymentsofficial - v2.1.0 - Prestaworks AB
                                            } else if (name_payment === 'klarnapaymentsofficial') {
                                                if (compatible_modules['klarnapaymentsofficial'] <= '2.0.8') {
                                                    ...
                                                    for (let i = 0; i < klarna_options.length; i++) {
                                                        ...
                                                        if ($('input[data-module-name=klarnapayments_' + payment_category + '_module]').is(':checked')) {
                                                            makeAuthorizeCall(payment_category);
                                                            Fronted.loadingBig(false);
                                                        }
                                                    }
                                                }
                                                ...
                                                if (name_subpayment == 'ps_checkout_hostedFields') {
                                                    AppOPC.$opc_step_three.find('#hostedFieldsErrors').addClass('hide-paypal-error');
                                                    AppOPC.$opc_step_three.find('#hostedFieldsErrors #hostedFieldsErrorList').html('');

                                                    $form_payment_selected.submit();

                                                    const interval = setInterval(() => {
                                                        if (AppOPC.$opc_step_three.find('#hostedFieldsErrors #hostedFieldsErrorList').length > 0) {
                                                            Fronted.loadingBig(false);
                                                            clearInterval(interval);
                                                        }
                                                    }, 2000);
                                                } else if (name_subpayment == 'ps_checkout_paypal') {
                                                    var callback = function () {
                                                        ...
                                                        }).render('#opc_ps_checkout_paypal .modal-body');
                                                    };

                                                    Fronted.showModal({
                                                        name: 'opc_ps_checkout_paypal',
                                                        type: 'normal',
                                                        button_close: true,
                                                        callback: callback
                                                    });
                                            ...
                                            } else if (name_payment === 'paytpv') {
                                                $('#exec_directpay').trigger('click');
                                            } else {
                                    ...
                                    checkOffer(callback_checkOffer);
                                };
...
var OPC_Compatibilities = {
    ...
    init: function () {
        ...
        //Support module: paypalplus - V1.7.13 de terracode*/
